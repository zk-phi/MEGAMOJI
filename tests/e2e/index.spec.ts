...
test("シンプルなテキスト絵文字を作成して、ダウンロードできる", async ({ page }) => {
  ...
  // プレビュー画像が更新される
  const src = await page.locator("img.result-cell").evaluate((el) => (el as HTMLImageElement).src);
...
test("絵文字フォントを使用した絵文字を作成して、ダウンロードできる", async ({ page }) => {
  ...
  // 生成待ち
  await page.locator("img.result-cell").evaluate((el) => (el as HTMLImageElement).src);
